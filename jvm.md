...
__Все примитивы храняться в стеке.__

| Type     | Size(bits)  | Minimum                                                     | Maximum                                                                    | Default
| -------- | --------    | --------                                                    | --------                                                                   | --------
| byte     | 8           | -2<sup>7</sup> (-128)                                       | 2<sup>7</sup>-1 (127)                                                      | 0
| short    | 16          | -2<sup>15</sup> (-32 768)                                   | 2<sup>15</sup>-1 (32 767)                                                  | 0
| int      | 32          | -2<sup>31</sup> (-2 147 483 648)                            | 2<sup>31</sup>-1 (2 147 483 647)                                           | 0
| long     | 64          | -2<sup>63</sup> (-9 223 372 036 854 775 808)                | 2<sup>63</sup>-1 (9,223,372,036,854,775,807)                               | 0
| float    | 32          | -2<sup>-149</sup> (1.40239846 x 10<sup>-45</sup>)           | 2·2<sup>-23</sup>·2<sup>127</sup> (3.40282347 x 10<sup>38</sup>)           | 0
| double   | 64          | -2<sup>-1074</sup> (4.9406564584124654 x 10<sup>-324</sup>) | 2·2<sup>-52</sup>·2<sup>1023</sup> (1.7976931348623157 x 10<sup>308</sup>) | 0
| char     | 16          | 0                                                           | 2<sup>16</sup>-1 (65 535)                                                  | 0 (/u0000’)
| boolean  | 1           | -                                                           | -                                                                          | false

`float` - число одинарной точности. При преодолении 6 десятичных знаков число становится менее точным.

`double` - число двойной точности. При преодолении 15 десятичных знаков число становится менее точным.

Если не применить `f` обозначение к литералу по умолчанию десятичное число - `double`.

Для точных вычислений двоичных чисел реконмендуемо использование `BigDecimal`.

**Типы ссылок и значения (Reference Data Types in Java)**

Существует 5 ссылочных типов: 
+ класс
+ массив (структура фиксированого размера елементов одного типа)
+ аннотация
+ интерфейс
+ перечисление (специальный типобезопасный вид класса)  

Их значения являются ссылками на динамически создаваемые экземпляры.

Java по умолчанию выделяет 4/8 байт для каждого ссылочного типа в зависимости от разрядности процессора. 
...
* Bootstrap/primordial class loader (Загрузчик класса Bootstrap)
* Extensions class loader (Загрузчик класса расширений)
* System class loader (Системный загрузчик классов)

**Загрузчик класса Bootstrap/primordial** загружает основные библиотеки Java, расположенные в папке `<JAVA_HOME>/jre/lib`. 
Этот загрузчик является частью ядра JVM, написан на нативном коде. Являеться родителем для остальных.
