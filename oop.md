...
Основные (major):
+ _Инкапсуляция_ (_Encapsulation_) - сокрытие реализации.
+ _Наследование_ (_Inheritance_) - создание новой сущности на базе уже существующей.
+ _Полиморфизм_ (_Polymorphism_) - возможность иметь разные формы для одной и той же сущности.
+ _Абстракция_ (_Abstraction_) - набор общих характеристик.
...
__Реализация в Java:__
Использование модификаторов доступа (private, protected, public) для контроля видимости переменных и методов.
...
__Реализация в Java:__
Использования наследования класса (__extends__) или имплементации интерфейса (__implements__)
...
>Любое обучение вождению не имело бы смысла, если бы человек, научившийся водить, скажем, ЗАЗ не мог потом водить Opel или BMW X3. С другой стороны, трудно представить человека, который смог бы нормально управлять автомобилем, в котором педаль газа находится левее педали тормоза, а вместо руля – джойстик. 
...
Если человеку надо доехать с работы до дома, то он сядет за руль автомобиля и будет выполнять одни и те же действия, независимо от того, какой именно тип автомобиля он использует. По сути, можно сказать, что все автомобили имеют один и тот же интерфейс, а водитель, абстрагируясь от сущности автомобиля, работает именно с этим интерфейсом. Если водителю предстоит ехать по немецкому автобану, он, вероятно выберет быстрый автомобиль с низкой посадкой, а для поездок по бездорожью гор - армейский пикап с высокой посадкой и хорошей проходимостью. Но, независимо от того, каким образом будет реализовываться движение и внутреннее функционирование машины, интерфейс останется прежним.
...
__Реализация в Java:__
Наследование от общего класса или имплементация интерфейса и использование их в реализации.
...
__Реализация в Java:__
Вынесение общей логики в родительский класс или в интерфейсы.
...
## Расскажите про основные понятия ООП (main concepts): _«класс»_, _«объект»_, _«интерфейс»_.
...
## Приципы взаимоотношения между класами в ООП _«IS-A»_ и _«HAS-A»_?
+ __«IS-A»__ - подразумевает наследование или реализацию интерфейсов.
+ __«HAS-A»__ - класс содержит обьект другого класа как часть своей структуры. Подразумевает __агрегацию__ или __композицию__.

[к оглавлению](#ООП)

## В чем разница между _асоциацией_, _композицией_ и _агрегацией_?
__Ассоциация__ обозначает связь между объектами. Композиция и агрегация — частные случаи ассоциации «часть-целое». Обьекты могут ссылаться друг на друга. Асоциация может быть односторонней или двусторнней.

__Агрегация__ предполагает, что объекты связаны взаимоотношением «_part-of_» (один обьект - часть другого). 

__Композиция__ более строгий вариант агрегации. Дополнительно к требованию «part-of» накладывается условие, что экземпляр другого обьекта не может принадлежать другому обьекту. 
...
## Что такое _статическое_ и _динамическое связывание_ (static and dynamic binding)?
Присоединение вызова метода к телу метода называется связыванием (__binding__). 

Если связывание проводится компилятором (компоновщиком) перед запуском программы (_compile time_), то оно называется __статическим__ или __ранним связыванием__ (__static__ or __early__ binding).
Связывание всех статичных, приватных и финальных методов происходит во время компиляции. 

В свою очередь, __позднее связывание, динамичное связывание или связыванием на стадии выполнения (late, dynamic or runtime binding)__ это связывание, проводимое непосредственно во время выполнения программы (__runtime__), в зависимости от типа объекта. В языках, реализующих позднее связывание, должен существовать механизм определения фактического типа объекта во время работы программы, для вызова подходящего метода. Иначе говоря, компилятор не знает тип объекта, но механизм вызова методов определяет его и вызывает соответствующее тело метода. Механизм позднего связывания зависит от конкретного языка, но нетрудно предположить, что для его реализации в объекты должна включаться какая-то дополнительная информация.
